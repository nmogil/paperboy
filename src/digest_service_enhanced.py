...
# Structured output schemas for HTML generation
class TLDRItem(BaseModel):
    """Schema for TL;DR summary items."""
    summary: str = Field(description="Brief summary with bold title and key point")
    relevance_score: int = Field(ge=0, le=100, description="Relevance score")
    type: str = Field(description="Type: 'paper' or 'news'")

class ArticleSection(BaseModel):
    """Schema for individual article in digest."""
    title: str = Field(description="Article title")
    article_type: str = Field(description="'NEWS' or 'RESEARCH'")
    priority: str = Field(description="'HIGH PRIORITY', 'IMPORTANT', or 'NOTEWORTHY'")
    score: int = Field(ge=0, le=100, description="Relevance score")
    summary: str = Field(description="Main summary text")
    relevance: str = Field(description="Why this matters to the user")
    key_insight: str = Field(description="Key insight or finding")
...
class QuickScanItem(BaseModel):
    """Schema for quick scan items."""
    icon: str = Field(description="📰 for news, 📄 for papers")
    title: str = Field(description="Truncated title (max 80 chars)")
    score: int = Field(ge=0, le=100, description="Relevance score")
    url: str = Field(description="Article URL")

class DigestHTML(BaseModel):
    """Schema for complete HTML digest."""
    header_title: str = Field(description="Main digest title")
    header_date: str = Field(description="Current date")
    header_subtitle: str = Field(description="Personalized subtitle with user name and title")
    header_stats: str = Field(description="Statistics line (papers, news, reading time)")
    
    tldr_items: List[TLDRItem] = Field(description="Today's highlights")
    
    priority_section_title: str = Field(description="Priority section title")
    priority_articles: List[ArticleSection] = Field(description="High priority articles")
    
    interesting_section_title: str = Field(description="Interesting section title") 
    interesting_articles: List[ArticleSection] = Field(description="Interesting articles")
    
    quick_scan_items: List[QuickScanItem] = Field(description="Quick scan items")
    
    footer_impact: str = Field(description="Research impact summary")
    footer_credits: str = Field(description="Generated by credits")
...
class EnhancedDigestService:
    ...
    async def generate_digest(self, task_id: str, user_info: Dict[str, Any], callback_url: str = None, target_date: str = None, top_n_articles: int = None, digest_sources: Optional[Dict[str, bool]] = None) -> None:
        ...
        try:
            ...
            digest_html = await self._generate_html(analyzed_articles, user_info)
...
{json.dumps(news_articles, indent=2)}"""
    ...
    async def _generate_html(self, articles: List[ArticleAnalysis], user_info: Dict[str, Any]) -> str:
        ...
        try:
            ...
            # Prepare articles data for structured generation
            articles_data = []
            for article in articles:
                article_data = {
                    'title': article.title,
                    'type': 'news' if article.type == ContentType.NEWS else 'paper',
                    'relevance_score': article.relevance_score,
                    'summary': article.summary,
                    'importance': article.importance,
                    'relevance_to_user': article.relevance_to_user,
            ...
            # Try structured HTML generation first
            try:
                structured_html = await self.llm_client.generate_html_digest(
                    articles_data,
                    user_info,
                    len(papers),
                    len(news),
                ...
                if structured_html:
                    logfire.info("Successfully generated HTML using structured outputs")
                    return structured_html
                else:
                    logfire.info("Structured HTML generation returned None, falling back to template")
                    
            except Exception as e:
                logfire.warning(f"Structured HTML generation failed, falling back to template: {str(e)}")

            # Fallback to current template-based generation
            logfire.info("Using template-based HTML generation as fallback")
